...
def process_blob(data, directory, latitude, longitude):
    m = str(data, "utf-8")
    m_split = m.split(' ')
    print("Matlab sent : " + m)
    command = m_split[0]
    if command == 'blob':
        ...
        if confidence > .85 and shape != 'nas':
            ...
            dictionary = {'type': 'standard', 'data': str(encoded_img), 'latitude': latitude, 'longitude': longitude,
                          'orientation': 'n', 'shape': shape, 'background_color': primary, 'alphanumeric': char[0],
                          'alphanumeric_color': secondary,
                          'autonomous': True}
...
while True:
    ...
    while True:
        ...
        if message_words[0] == "dir":
            ...
            # lol = {"processed": True}
            # patch_url = "http://192.168.0.13:25005/api/images/" + str(img_id)
